...
class CasbinService:
    ...
    def _initialize_enforcer(self) -> None:
        ...
        try:
            ...
            model_path = os.path.join(os.path.dirname(__file__), "..", "config", "casbin_model.conf")
    ...
    def assign_role(
        ...
        try:
            ...
            if success:
                ...
                self.logger.info(f"Role {role} assigned to user {user_id} in domain {domain}")
    ...
    def remove_role(
        ...
        try:
            if domain:
                success = self.enforcer.delete_role_for_user_in_domain(user_id, role, domain)
            else:
                success = self.enforcer.delete_role_for_user(user_id, role)
            
            if success:
                self.enforcer.save_policy()
                self.logger.info(f"Role {role} removed from user {user_id} in domain {domain}")
    ...
    def add_policy(
        ...
        try:
            ...
            if success:
                self.enforcer.save_policy()
                self.logger.info(f"Policy added: {subject} -> {obj} -> {action} in domain {domain}")
    ...
    def remove_policy(
        ...
        try:
            ...
            if success:
                self.enforcer.save_policy()
                self.logger.info(f"Policy removed: {subject} -> {obj} -> {action} in domain {domain}")
