...
class MembershipService:
    ...
    def get_memberships_by_role(self, role_id: UUID) -> List[Membership]:
        """Get all memberships for a specific role."""
        return self.db.query(Membership).filter(Membership.role_id == role_id).all()

    def get_membership_by_user_and_role(
        self, user_id: UUID, role_id: UUID
    ) -> Optional[Membership]:
        """Get a membership by user ID and role ID."""
        return (
    ...
    def get_memberships(self, skip: int = 0, limit: int = 100) -> List[Membership]:
        """Get a list of memberships with pagination."""
        return self.db.query(Membership).offset(skip).limit(limit).all()
    ...
    def get_memberships_query(self) -> Query:
        ...
        Returns:
            Query: SQLAlchemy query object for memberships.
        """
        return self.db.query(Membership)
    ...
    def search(self, filters: dict) -> List[Membership]:
        """
        Search memberships based on dynamic filter criteria.
        ...
        Returns:
            List[Membership]: Filtered list of memberships matching the criteria.
        """
